...
public class JinqJPATypesTest extends JinqJPATestBase
{
   ...
   @Test(expected=ClassCastException.class)  // This used to work in EclipseLink 2.7.3, but in 2.7.7, this query now returns a BigDecimal for some reason
   ...
   public void testCustomFunction() throws NoSuchMethodException, SecurityException
   {
      ...
      List<String> customers = streams.streamAll(em, Customer.class)
            ...
            .select(c -> c.getName())
            .toList();
      assertEquals("SELECT A.name FROM Customer A WHERE function('isRegexMatch', A.name, 'A.*') = 1", query);
      assertEquals(1, customers.size());
