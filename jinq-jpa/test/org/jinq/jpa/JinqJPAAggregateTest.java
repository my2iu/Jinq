...
public class JinqJPAAggregateTest extends JinqJPATestBase
{
   ...
   public void testCountSortedInSubquery()
   {
      ...
      long count = streams.streamAll(em, Customer.class)
            ...
            .sortedBy(c -> c.getName())
            .count();
      assertEquals("SELECT A FROM Customer A WHERE (SELECT COUNT(B) FROM A.sales B) > 1", query);
   ...
   public void testCountSortedInGroup()
   {
      ...
      List<Pair<String, Long>> customers = streams.streamAll(em, Customer.class)
            ...
            .toList();
      assertEquals("SELECT A.country, COUNT(A) FROM Customer A GROUP BY A.country", query);
   ...
   public void testGroupLots()
   {
      ...
      assertEquals("SELECT A.country, COUNT(A), COUNT(A), COUNT(A), COUNT(A), COUNT(A), COUNT(A), MIN(A.salary) FROM Customer A GROUP BY A.country", query);
   }
