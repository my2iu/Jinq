...
class QueryJPAJinqStream<T> extends QueryJinqStream<T> implements JPAJinqStream<T> 
{
   ...
   {
      super(query);
      if (!(query instanceof JPAQueryComposer)) throw new IllegalArgumentException("Cannot make a JPA stream without a JPA Query Composer");
   ...
   {
      QueryComposer<T> newComposer = jpaComposer.andNotExcept(otherSet);
      if (newComposer != null) return makeQueryStream(newComposer, inQueryStreamSource);
      return new JPAJinqStreamWrapper<>(this).andNotExcept(otherSet);
   }
