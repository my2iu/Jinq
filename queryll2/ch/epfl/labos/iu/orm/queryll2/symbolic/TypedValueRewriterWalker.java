...
public class TypedValueRewriterWalker<I> extends TypedValuePostfixWalker<I, TypedValue>
{
   HashMap<TypedValue, TypedValue> remap;  // to handle aliasing, we need to track changes we made elsewhere in the tree
   TypedValueVisitor<I, TypedValue> rewriter;
   ...
   public TypedValueRewriterWalker(TypedValueVisitor<I, TypedValue> rewriter)
   {
      this.rewriter = rewriter;
      this.remap = new HashMap<TypedValue, TypedValue>();
   }
   
   @Override public TypedValue defaultValue(TypedValue val, I in) throws TypedValueVisitorException
   {
   ...
   @Override public TypedValue unaryOpValue(TypedValue.UnaryOperationValue val, I in) throws TypedValueVisitorException
   {
      if (remap.containsKey(val)) return remap.get(val);
...
//   }
   ...
   @Override public TypedValue binaryOpValue(TypedValue.BinaryOperationValue val, I in) throws TypedValueVisitorException
   {
      if (remap.containsKey(val)) return remap.get(val);
...
//   }
   ...
   @Override public TypedValue methodCallValue(MethodCallValue val, I in) throws TypedValueVisitorException
   {
      if (remap.containsKey(val)) return remap.get(val);
   ...
   @Override public TypedValue staticMethodCallValue(MethodCallValue.StaticMethodCallValue val, I in) throws TypedValueVisitorException
   {
      if (remap.containsKey(val)) return remap.get(val);
      MethodCallValue newVal = val;
      List<TypedValue> newArgs = new Vector<TypedValue>(val.args.size());
   ...
   @Override public TypedValue virtualMethodCallValue(MethodCallValue.VirtualMethodCallValue val, I in) throws TypedValueVisitorException
   {
      if (remap.containsKey(val)) return remap.get(val);
      MethodCallValue newVal = val;
      List<TypedValue> newArgs = new Vector<TypedValue>(val.args.size());
