...
//         System.out.println(p);
   ...
   public void testAndNotExcept()
   {
      ...
      List<String> customers = base.andNotExcept(base)
            ...
            .sortedBy(name -> name)
            .toList();
      assertEquals("SELECT A.name FROM org.jinq.hibernate.test.entities.Customer A WHERE 1 = 0 ORDER BY A.name ASC", query);
      ...
      customers = base.andNotExcept(base.where(c -> c.getDebt() < 250))
            ...
            .sortedBy(name -> name)
            .toList();
      assertEquals("SELECT A.name FROM org.jinq.hibernate.test.entities.Customer A WHERE NOT A.debt < 250 ORDER BY A.name ASC", query);
      assertEquals(1, customers.size());
      ...
      customers = base.where(c -> c.getDebt() < 250)
            ...
            .sortedBy(name -> name)
            .toList();
      assertEquals("SELECT A.name FROM org.jinq.hibernate.test.entities.Customer A WHERE A.debt < 250 AND NOT A.debt > 150 ORDER BY A.name ASC", query);
